...
def cross_validate(data_x, data_y, n_splits=2):
	...
	print "Avg SVM score:", svm_avg/n_splits
	print "Avg KNN score:", knn_avg/n_splits

def evaluate_knn_classifier(test_input, test_output):
	...
	print("Accuracy: ", accuracy_score(test_output, test_prediction))
	conf_matrix = confusion_matrix(test_output, test_prediction)
	print("Confusion Matrix: ", conf_matrix)

def evaluate_svm_classifier(test_input, test_output):
...
def get_data_set(filename):
	...
	with open(filename,'rb') as csvfile:
		...
		for row in reader:
			...
			if activity > 0:
				raw_data.append([float(row['acc1_X']), float(row['acc1_Y']), float(row['acc1_Z']), float(row['acc2_X']), float(row['acc2_Y']), float(row['acc2_Z']), float(row['gyro2_X']), float(row['gyro2_Y']), float(row['gyro2_Z']),float(row['acc3_X']), float(row['acc3_Y']), float(row['acc3_Z']), float(row['gyro3_X']), float(row['gyro3_Y']), float(row['gyro3_Z']), activity])
				# raw_y.append(activity)
	csvfile.close()

	# raw_x = imputer.fit_transform(raw_x)
...
train_input, train_output, test_input, test_output = get_data_set(filename='MHEALTHDATASET/data.csv')
train_svm_model(train_input, train_output)
evaluate_svm_classifier(test_input, test_output)
evaluate_knn_classifier(test_input, test_output)
